...
def test_class_init_two_mandatory_parameters() -> None:
    ...
    assert "Handler.__init__ should have" in str(excinfo.value)
...
def test_partial_class_multiple_mandatory_parameters() -> None:
    ...
    partial_handler_invalid = partial(Handler, "before")
    handler_invalid = create_handler(partial_handler_invalid)
    with pytest.raises(TypeError) as excinfo:
        list(handler_invalid(nodes[0]))

    assert "Handler.__init__ should have" in str(excinfo.value)
    assert "node, after" in str(excinfo.value)
    assert "Add a default value for dataclass fields" not in str(excinfo.value)
